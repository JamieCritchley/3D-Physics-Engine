...
namespace VisualDebugger
{
	...
	///simulation objects
	Camera* camera;
	PhysicsEngine::AssignmentScene::PenaltyScene* scene;
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		scene = new PhysicsEngine::AssignmentScene::PenaltyScene(&hud);
		scene->Init();
		...
		//Limits update speed to be a constant 60 fps (and not based on monitor refresh rate)
		DEVMODE* montitorRefreshRate = new DEVMODE();
		EnumDisplaySettingsA(nullptr, ENUM_CURRENT_SETTINGS, montitorRefreshRate);
		delay = delta_time * 1000 - (delta_time / (( (float)montitorRefreshRate->dmDisplayFrequency)/60)) * 1000; //In milliseconds
	...
	void UserKeyRelease(int key)
	{
		...
		//User kicks ball on relesease of key after holding for power
		case 'T':
			scene->PlayerKick();
			break;
	...
	void UserKeyHold(int key)
	{
		...
		//PenaltyScene implementation

		//Rotations
		case 'F':
			scene->RotatePlayer(-.01);
		...
		//Increment kicking power (hold time measures power)
		case 'T':
			scene->IncrementKickPower(.5);
